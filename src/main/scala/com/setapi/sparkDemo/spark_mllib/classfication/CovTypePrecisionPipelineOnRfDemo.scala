...
object CovTypePrecisionPipelineOnRfDemo {
  def main(args: Array[String]): Unit = {
    // TODO: 构建 SparkSession 实例
    val spark = SparkSession.builder()
      .appName("CovTypePrecisionPipelineOnRfDemo")
      .master("local[3]")
      .getOrCreate()
    ...
    println("================   分割线   =================")
    println("============================================")
    println("进一步使用随机森林作优化模型")
    randomForestWithUncodingData(spark, uncodingData)
  ...
  // 同时变更使用随机森林决策树模型
  private def randomForestWithUncodingData(spark: SparkSession, uncodingData: DataFrame) = {
    val Array(uncodingTrainingData, uncodingTestingData) = uncodingData.randomSplit(Array(0.9, 0.1))
    uncodingTrainingData.cache()
    uncodingTestingData.cache()
