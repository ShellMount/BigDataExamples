...
object TitanicClassification3 {
  def main(args: Array[String]): Unit = {
    // TODO: 构建 SparkSession 实例
    val spark = SparkSession.builder()
      .appName("TitanicClassification3")
      .master("local[3]")
      .getOrCreate()
    ...
    // TODO: age one-hot 化
    val ageCategoryMap = titanicDF
      .select("Age")
      .rdd
      .map(row => {
        // 年龄为空时使用平均年龄
        val ageValue = if (row.get(0) == null) aveAge else row.getDouble(0)
        // 按照年龄划分不同段
        if (ageValue >= 61) {
          "old"
        } else if (ageValue >= 26) {
      ...
      .collectAsMap()
    // 广播年龄集合
    val ageCategoryMapBroadcast = sc.broadcast(ageCategoryMap)
    ...
    val titanicRDD = titanicDF
      ...
      .map(row => {
        ...
        // TODO: Age处理
        val ageValue = if (row.get(3) == null) aveAge else row.getDouble(3)
        val ageCategory = {
          if (ageValue >= 61) {
            "old"
          } else if (ageValue >= 26) {
        ...
        val ageFeatureArray = new Array[Double](ageCategoryMapBroadcast.value.size)
        ageFeatureArray(ageCategoryMapBroadcast.value(ageCategory).toInt) = 1.0
        ...
        val features = Vectors.dense(
          Array(row.getInt(1).toDouble,
            ...
            ++ ageFeatureArray    // TODO: 年龄特征值 one-hot 化
        )
