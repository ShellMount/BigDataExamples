...
object BikeSharingRegression {
  def main(args: Array[String]): Unit = {
    ...
    // 决策树算法中，categoricalFeaturesInfo的特征值需要从0开始，故下记 值 - 1，并记录该特征值的数量
    ...
    /**
      * 随机森林算法中，每一棵树是使用不同的数据集针对决策对算法训练得到的模型
      * 不同数据集：
      * 主要区别在于，数据集的条目一至的，但是每条数据的特征数是不一样的，是原始特征的一部分。
      * 由参数控制：auto, all, sqrt, log2, onethird
      */
    val rfrModel = RandomForest.trainRegressor(
      trainingRDD,
      Map[Int, Int](
        0 -> 4, // 第 n 个特征类 有 k 个取值, 其取值从 0 开始： 0 ~ k-1
        1 -> 2,
    ...
    // 使用模型预测
    val rfrPredictAndActualRDD = testRDD.map(lp => {
      val predictLabel = rfrModel.predict(lp.features)
      (predictLabel, lp.label)
    })

    println("随机森林下真实值与预测值: ")
    println(rfrPredictAndActualRDD.take(10).foreach(println))

    // 回归模型的性能评估指标
    val rfrMetrics = new RegressionMetrics(rfrPredictAndActualRDD)
    println(s"随机森林回归模型的评估，均方根误差RMSE: ${rfrMetrics.rootMeanSquaredError}")
    println(s"随机森林回归模型的评估，均方误差MSE: ${rfrMetrics.meanSquaredError}")
    println(s"随机森林回归模型的评估，平均平均绝对误差: ${rfrMetrics.meanAbsoluteError}")
    println(s"随机森林回归模型的评估，R2: ${rfrMetrics.r2}")
    println(s"随机森林回归模型的评估，Variance: ${rfrMetrics.explainedVariance}")
